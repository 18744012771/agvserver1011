...
void TcpSession::send(const Json::Value &json)
{
    ...
    char *send_buffer = new char[length + 6];//+6???
    memset(send_buffer, 0, length + 6);//将一块内存全部初始化为０
    send_buffer[0] = MSG_MSG_HEAD;//head
    memcpy_s(send_buffer + 1, length+5, (char *)&length, sizeof(length));//???length
    memcpy_s(send_buffer +5, length + 1, msg.c_str(),msg.length());//?????length + 1 多出来一个字节

    int per_send_length = MSG_READ_BUFFER_LENGTH;

    if(length+5<per_send_length){//在内存中直接读取
        //if(length+5 !=  boost::asio::write(socket_, boost::asio::buffer(data, len)));
        if(length+5 != boost::asio::write(socket_,boost::asio::buffer(send_buffer,length+5)))//write protocol head+length+msg
        {
            combined_logger->warn("send error ") ;
        }
    }else{//分批发送
        int packindex = 0;
        char *per_send_buffer = new char[per_send_length];//开辟一个最大读取长度的内存
        int resendTime = 3;
        while(true){
            int this_time_send_length = (length +5) - packindex * per_send_length;
            if(this_time_send_length<=0)break;
            if(this_time_send_length > per_send_length)this_time_send_length = per_send_length;//赋值最大发送长度

            memcpy(per_send_buffer,send_buffer+packindex*per_send_length,this_time_send_length);
            bool sendRet = false;
            for(int i=0;i<resendTime;++i){// 发送尝试，成功返回，不成功尝试三次
...
void TcpSession::start()
{
    ...
    if(GLOBAL_AGV_PROJECT == AGV_PROJECT_ANTING || GLOBAL_AGV_PROJECT == AGV_PROJECT_DONGYAO){
        if(getAcceptID() == AgvManager::getInstance()->getServerAccepterID()){//在哪设置的accepterID
    ...
    std::thread([this](){
        while(socket_.is_open()){
            ...
            size_t length = socket_.read_some(boost::asio::buffer(read_buffer,MSG_READ_BUFFER_LENGTH), error);//1024
...
int TcpSession::ProtocolProcess()
{
    ...
    if(EndIndex>StartIndex)
    {
        ...
        if(FrameData.find('*') != std::string::npos)//npos 是未找到
        ...
        if(pack_head>=0){
            ...
            if(json_len==0){
                ...
                if(json_len <= buffer.length()-1-sizeof(int32_t))
                {
                    ...
                    if (reader.parse(sss, root))//sss转换为root
                    {
