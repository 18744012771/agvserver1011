...
void rosAgv::goStation(std::vector<int> lines,  bool stop)
{
    ...
    for (auto line : lines) {
        mapElement *Element = MapManager::getInstance()->getMapElementById(line);
        if (Element == nullptr || Element->getElementType() != mapElement::Map_Element_Type_Path)
            continue;
        ...
        //获取站点：
        mapElement *Element_start = MapManager::getInstance()->getMapElementById(startId);
        if (Element_start == nullptr || Element_start->getElementType() != mapElement::Map_Element_Type_Point)
            continue;

        mapElement *Element_end = MapManager::getInstance()->getMapElementById(endId);
        if (Element_end == nullptr || Element_end->getElementType() != mapElement::Map_Element_Type_Point)
            continue;

        start = static_cast<MapPoint *>(Element_start);
        start->getRealX();
        start->getName();
        start->getRealY();

        end = static_cast<MapPoint *>(Element_end);
        end->getRealX();
        end->getName();
        end->getRealY();
...
void rosAgv::excutePath(std::vector<int> lines)
{
    ...
    for (auto line : lines) {
        mapElement *Element = MapManager::getInstance()->getMapElementById(line);
        if (Element == nullptr || Element->getElementType() != mapElement::Map_Element_Type_Path)continue;
